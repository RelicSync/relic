...
void main() {
  ...
  test(
    'a double tap latches even when the worker reports idle between taps',
    () async {
      ...
      final voice = VoiceController(
        repo,
        launchProcess: (_, _) async => worker,
      );
      ...
      expect(
        voice.maxSeconds,
        600,
        reason: 'the worker says how long a take may run',
      );
  ...
  test(
    'the Voice key can be changed, is saved, and every label follows it',
    () async {
      final profile = Platform.environment['RELIC_DATA_DIR'];
      ...
      binding.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        ...
        calls.add(call);
        return call.method == 'enable' || call.method == 'key' ? true : null;
      });
      addTearDown(
        () => binding.defaultBinaryMessenger.setMockMethodCallHandler(
          channel,
      ...
      List<Object?> keys() => calls
          .where((c) => c.method == 'key')
          .map((c) => c.arguments)
          .toList();

      Future<(VoiceController, _FakeWorker, LocalDeskRepo)> launch() async {
        final worker = _FakeWorker();
        final repo = LocalDeskRepo();
        final voice = VoiceController(
          repo,
        ...
        await voice.initialize();
        worker.emit({'event': 'hello', 'protocol': 1, 'backend': 'cpu'});
        worker.emit({'event': 'ready', 'devices': []});
        await settle();
        return (voice, worker, repo);
      }

      var (voice, worker, repo) = await launch();
      expect(voice.keyId, 'right_alt', reason: 'Right Alt stays the default');
      expect(VoiceController.keyLabel, 'Right Alt');
      expect(keys(), [0xA5]);
      final enable = calls.lastIndexWhere((c) => c.method == 'enable');
      expect(
        enable,
        lessThan(calls.lastIndexWhere((c) => c.method == 'key')),
        reason: 'the hook restarts first, then learns its key',
      );

      calls.clear();
      await voice.setKey('caps_lock');
      expect(keys(), [0x14]);
      expect(VoiceController.keyLabel, 'Caps Lock');
      expect(await prefs.readAsString(), contains('"key":"caps_lock"'));

      calls.clear();
      await voice.setKey('not_a_key');
      expect(keys(), isEmpty, reason: 'unknown keys are ignored');
      expect(voice.keyId, 'caps_lock');

      worker.kill();
      voice.dispose();
      repo.dispose();

      // A restart comes back on the saved key.
      calls.clear();
      (voice, worker, repo) = await launch();
      expect(voice.keyId, 'caps_lock');
      expect(keys(), [0x14]);
      worker.kill();
      voice.dispose();
