// The EFF short wordlist 2.0 (1,296 words), by the Electronic Frontier
// Foundation. Each word has a unique three-character prefix and an edit
// distance of at least 3 from every other word, which makes typed diceware
// phrases resilient to typos and autocomplete. Public domain (CC0).
// Source: https://www.eff.org/dice ("eff_short_wordlist_2_0.txt").
//
// Used to (a) generate suggested vault passphrases via a CSPRNG and (b) detect
// diceware phrases when estimating passphrase strength. Do not reorder or edit:
// the list is a fixed reference, and 1,296 = 6^4 keeps the per-word entropy at
// exactly log2(1296) bits.

/// The 1,296 words of the EFF short wordlist 2.0, lowercase, in list order.
...
/// A fast membership set over [effShortWordlist], for diceware detection.
final Set<String> effShortWordSet = effShortWordlist.toSet();
