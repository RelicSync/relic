...
/// Opens the system browser to Supabase's authorize URL and catches the callback:
///   - Desktop: a one-shot 127.0.0.1 loopback HTTP server (no OS registration).
///   - Mobile:  the `relic://auth-callback` deep link, delivered by mobile.dart's
///     app_links handler via [deliverMobileCallback].
///
...
class OAuthFlow {
  ...
  // Mobile: the in-flight login's callback sink, fed by deliverMobileCallback.
  ...
  // --- Mobile: relic://auth-callback deep link ------------------------------
  static Future<SupabaseSession> _mobile(
    ...
    try {
      const redirect = 'relic://auth-callback';
      final authorize = SupabaseAuth.authorizeUrl(provider, redirect, challenge);
      if (!await launchUrl(authorize, mode: LaunchMode.externalApplication)) {
        throw 'Could not open the browser.';
      }
